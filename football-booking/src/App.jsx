    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center py-4">
        ...
        <nav className="hidden md:flex space-x-6">
          {currentUser?.phone === 'admin' ? (<span className="text-red-600 font-bold uppercase">Trang Quản Trị Viên</span>) : (
              <><button onClick={() => setCurrentView('home')} className="text-gray-700 hover:text-green-600 font-medium">Trang chủ</button><button onClick={() => setCurrentView('search')} className="text-gray-700 hover:text-green-600 font-medium">Tìm sân</button><button className="text-gray-700 hover:text-green-600 font-medium">Khuyến mãi</button><button className="text-gray-700 hover:text-green-600 font-medium">Liên hệ</button></>)}
        </nav>
        <div className="hidden md:flex items-center space-x-4">
          {isLoggedIn ? (
            <><span className="text-sm font-bold text-green-700">Hi, {currentUser?.name || 'Bạn'}</span>
              {currentUser?.phone !== 'admin' && (<button onClick={() => setCurrentView('history')} className="text-gray-700 hover:text-green-600 flex items-center space-x-1"><History className="w-5 h-5" /> <span>Lịch sử</span></button>)}
          ...
          ) : (<button onClick={() => setCurrentView('login')} className="flex items-center space-x-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 font-bold"><LogIn className="w-4 h-4" /><span>Đăng nhập</span></button>)}
        </div>
...
const HomePage = ({ setCurrentView, fields, loading, setSelectedField }) => (
  <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center">
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-4xl mx-auto">
            <button onClick={() => setCurrentView('search')} className="bg-green-600 text-white rounded-lg px-8 py-3 font-bold hover:bg-green-700">Tìm sân ngay</button>
          </div>
        </div>
      </div>
    </div>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white rounded-lg shadow-md p-6 text-center hover:shadow-xl transition"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><Search className="w-8 h-8 text-green-600" /></div><h3 className="text-xl font-semibold mb-2 text-gray-800">Tìm kiếm thông minh</h3><p className="text-gray-600">Tìm sân theo vị trí, giờ, loại sân và mức giá phù hợp với bạn</p></div>
          <div className="bg-white rounded-lg shadow-md p-6 text-center hover:shadow-xl transition"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><Calendar className="w-8 h-8 text-green-600" /></div><h3 className="text-xl font-semibold mb-2 text-gray-800">Đặt sân dễ dàng</h3><p className="text-gray-600">Chọn giờ, thanh toán và nhận mã xác nhận chỉ trong vài phút</p></div>
          <div className="bg-white rounded-lg shadow-md p-6 text-center hover:shadow-xl transition"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><Star className="w-8 h-8 text-green-600" /></div><h3 className="text-xl font-semibold mb-2 text-gray-800">Đánh giá tin cậy</h3><p className="text-gray-600">Xem đánh giá từ cộng đồng để chọn sân tốt nhất</p></div>
        </div>
    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {fields.map(field => (
              <div key={field.SanID} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition cursor-pointer" onClick={() => { setSelectedField(field); setCurrentView('detail'); }}>
                <img src={field.HinhAnh} alt={field.TenSan} className="w-full h-48 object-cover" /><div className="p-4"><h3 className="text-xl font-semibold mb-2 text-gray-800">{field.TenSan}</h3><div className="flex items-center text-gray-600 mb-2"><MapPin className="w-4 h-4 mr-1" /><span className="text-sm">{field.DiaChi}</span></div><div className="flex items-center justify-between mb-2"><div className="flex items-center"><Star className="w-4 h-4 text-yellow-400 fill-current mr-1" /><span className="text-sm font-semibold">{field.DiemDanhGia}</span><span className="text-sm text-gray-500 ml-1">({field.SoLuotReview} đánh giá)</span></div><span className="text-sm text-gray-600">{field.LoaiSan}</span></div><div className="text-green-600 font-bold">Từ 180.000đ/giờ</div></div>
              </div>
            ))}
          </div>
        )}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <select className="border p-2 rounded"><option>Loại sân</option></select><select className="border p-2 rounded"><option>Giá</option></select><select className="border p-2 rounded"><option>Đánh giá</option></select><button className="bg-green-600 text-white rounded p-2">Áp dụng</button>
           </div>
      </div>
      <div className="grid grid-cols-1 gap-6">
            {fields.map(field => (
              <div key={field.SanID} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition cursor-pointer" onClick={() => { setSelectedField(field); setCurrentView('detail'); }}>
                <div className="md:flex"><img src={field.HinhAnh} className="w-full md:w-64 h-48 object-cover" /><div className="p-6 flex-1"><h3 className="text-2xl font-semibold text-gray-800">{field.TenSan}</h3><div className="flex items-center text-gray-600 mb-3"><MapPin className="w-4 h-4 mr-1" /><span>{field.DiaChi}</span></div><div className="flex justify-between items-center mt-4"><div className="text-green-600 font-bold text-xl">Đặt sân để xem giá</div><button className="bg-green-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-green-700">Xem chi tiết</button></div></div></div>
              </div>
            ))}
      </div>
    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <div className="space-y-3 mb-6"><div className="flex items-center text-gray-700"><MapPin className="w-5 h-5 mr-3 text-green-600" /><span>{selectedField.DiaChi}</span></div><div className="flex items-center text-gray-700"><Phone className="w-5 h-5 mr-3 text-green-600" /><span>Chủ sân: Phùng Vĩnh Phước - 0328665619</span></div></div>
                <h3 className="text-xl font-semibold mb-3 text-gray-800">Tiện ích</h3><div className="flex flex-wrap gap-2 mb-6">{selectedField.TienIch && selectedField.TienIch.split(',').map((t, i) => <span key={i} className="bg-green-50 text-green-700 px-4 py-2 rounded-lg border border-green-200">{t}</span>)}</div>
                <h3 className="text-xl font-semibold mb-3 text-gray-800 border-t pt-4">Đánh giá gần đây</h3><div className="space-y-3"><div className="bg-gray-50 p-3 rounded"><strong>Nguyễn Văn A:</strong> Sân đẹp, giá tốt! ⭐⭐⭐⭐⭐</div><div className="bg-gray-50 p-3 rounded"><strong>Trần Thị B:</strong> Chủ sân nhiệt tình. ⭐⭐⭐⭐</div></div>
            </div>
                  ...
                  <div className="space-y-4">
                      ...
                      {(isConflict || isPastConflict) && (
                          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
                              ...
                              {isPastConflict ? (<span className="block sm:inline"> Giờ này đã trôi qua. Vui lòng chọn giờ hiện tại hoặc tương lai.</span>) : (<span className="block sm:inline"> Giờ bạn chọn đã bị trùng cho {bookingInfo.fieldType}.</span>)}
                          </div>
                      )}
                      ...
                      <button onClick={() => { if (!isConflict && !isPastConflict && bookingInfo.totalPrice > 0 && bookingInfo.name && bookingInfo.phone) setCurrentView('payment'); else alert("Kiểm tra lại thông tin hoặc giờ đặt!"); }} disabled={isConflict || isPastConflict} className={`w-full text-white py-3 rounded-lg font-semibold transition ${(isConflict || isPastConflict) ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}>{isPastConflict ? 'Giờ đã qua' : isConflict ? 'Giờ này đã kín' : 'Tiếp tục thanh toán'}</button>
                  </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <div className="space-y-4">{authMode === 'register' && (<input type="text" className="w-full border border-gray-300 rounded-lg px-4 py-2" placeholder="Họ và tên" />)}<input type="text" className="w-full border border-gray-300 rounded-lg px-4 py-2" placeholder="Số điện thoại/Email" value={inputPhone} onChange={(e) => setInputPhone(e.target.value)} />{authMode !== 'forgot' && (<input type="password" className="w-full border border-gray-300 rounded-lg px-4 py-2" placeholder="Mật khẩu" />)}<button onClick={() => { if (authMode === 'register') { alert('Đăng ký thành công! Vui lòng đăng nhập.'); setAuthMode('login'); } else if (authMode === 'forgot') { alert('Mật khẩu mới đã gửi về email.'); setAuthMode('login'); } else { handleLogin(inputPhone); } }} className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700">{authMode === 'login' ? 'Đăng nhập' : authMode === 'register' ? 'Đăng ký' : 'Gửi lại mật khẩu'}</button><div className="text-center text-sm space-y-2 mt-4">{authMode === 'login' && (<><p onClick={() => setAuthMode('forgot')} className="text-green-600 cursor-pointer hover:underline">Quên mật khẩu?</p><p onClick={() => setAuthMode('register')} className="text-gray-600 cursor-pointer hover:text-green-600">Chưa có tài khoản? Đăng ký ngay</p></>)}{authMode !== 'login' && (<p onClick={() => setAuthMode('login')} className="text-green-600 cursor-pointer hover:underline">Quay lại đăng nhập</p>)}</div><button onClick={() => setCurrentView('home')} className="w-full text-gray-400 text-sm mt-2">Về trang chủ</button></div>
      </div>
    </div>
  );
};
...
const HistoryPage = ({ setCurrentView, history, clearHistory }) => (
  <div className="min-h-screen bg-gray-50 py-8">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">{history.length === 0 ? (<div className="p-10 text-center text-gray-500">Chưa có lịch sử đặt sân nào. Hãy đặt sân ngay!</div>) : (<div className="divide-y divide-gray-200">{history.map((item, index) => (<div key={index} className="p-6 hover:bg-gray-50 flex justify-between items-center"><div><h3 className="font-bold text-lg text-gray-800">{item.san} ({item.sanType || 'Sân 5'})</h3><p className="text-gray-600">Ngày: {item.ngay} | Giờ: {item.gio}</p><p className="text-sm text-gray-500">Người đặt: {item.nguoiDat} - {item.sdt}</p></div><div className="text-right"><div className="text-green-600 font-bold text-xl">{new Intl.NumberFormat('vi-VN').format(item.gia)}đ</div><span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">Đã thanh toán</span></div></div>))}</div>)}</div>
...
const AdminPage = ({ fields, onAddField, onDeleteField }) => {
  ...
  const [newField, setNewField] = useState({ TenSan: '', DiaChi: '', GiaTheoGio: 200000, LoaiSan: 'Sân 5', HinhAnh: '', MoTa: '', TienIch: '' });
  const handleSubmit = async () => { if(!newField.TenSan || !newField.DiaChi) return alert("Nhập đủ tên và địa chỉ!"); await onAddField(newField); setIsAdding(false); setNewField({ TenSan: '', DiaChi: '', GiaTheoGio: 200000, LoaiSan: 'Sân 5', HinhAnh: '', MoTa: '', TienIch: '' }); };
  return (
    <div className="min-h-screen bg-gray-100 py-8">
        <div className="max-w-7xl mx-auto px-4"><h1 className="text-3xl font-bold text-gray-800 mb-6">Trang Quản Lý Sân Bóng</h1><div className="mb-6"><button onClick={() => setIsAdding(!isAdding)} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold flex items-center">{isAdding ? <X className="w-5 h-5 mr-2"/> : <Plus className="w-5 h-5 mr-2"/>} {isAdding ? 'Hủy thêm mới' : 'Thêm sân bóng mới'}</button></div>
            {isAdding && (<div className="bg-white p-6 rounded-lg shadow-md mb-6 animate-fade-in"><h2 className="text-xl font-bold mb-4">Nhập thông tin sân</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input className="border p-2 rounded" placeholder="Tên sân" value={newField.TenSan} onChange={e => setNewField({...newField, TenSan: e.target.value})} /><input className="border p-2 rounded" placeholder="Địa chỉ" value={newField.DiaChi} onChange={e => setNewField({...newField, DiaChi: e.target.value})} /><input className="border p-2 rounded" type="number" placeholder="Giá/giờ" value={newField.GiaTheoGio} onChange={e => setNewField({...newField, GiaTheoGio: e.target.value})} /><select className="border p-2 rounded" value={newField.LoaiSan} onChange={e => setNewField({...newField, LoaiSan: e.target.value})}><option>Sân 5</option><option>Sân 7</option></select><input className="border p-2 rounded" placeholder="Link hình ảnh" value={newField.HinhAnh} onChange={e => setNewField({...newField, HinhAnh: e.target.value})} /><input className="border p-2 rounded" placeholder="Tiện ích" value={newField.TienIch} onChange={e => setNewField({...newField, TienIch: e.target.value})} /></div><textarea className="border p-2 rounded w-full mt-4" placeholder="Mô tả..." value={newField.MoTa} onChange={e => setNewField({...newField, MoTa: e.target.value})} /><button onClick={handleSubmit} className="mt-4 bg-green-600 text-white px-8 py-2 rounded font-bold">Lưu sân bóng</button></div>)}
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50"><tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tên sân</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Địa chỉ</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Giá</th><th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Thao tác</th></tr></thead>
                    <tbody className="bg-white divide-y divide-gray-200">{fields.map(field => (<tr key={field.SanID}><td className="px-6 py-4 whitespace-nowrap font-medium">{field.TenSan}</td><td className="px-6 py-4">{field.DiaChi}</td><td className="px-6 py-4 text-green-600 font-bold">{new Intl.NumberFormat('vi-VN').format(field.GiaTheoGio)}đ</td><td className="px-6 py-4 text-right"><button onClick={() => onDeleteField(field.SanID)} className="text-red-600 hover:text-red-900 font-bold flex items-center justify-end ml-auto"><Trash2 className="w-4 h-4 mr-1"/> Xóa</button></td></tr>))}</tbody>
                </table>
            </div>
            
            {/* BOOKING LIST FOR ADMIN  */}
            <h2 className="text-2xl font-bold text-gray-800 mt-10 mb-4">Danh Sách Lịch Đặt</h2>
            <AdminBookingsList />
        </div>
    </div>
  );
...
// Component con để hiển thị Lịch đặt (Tách ra để dùng useEffect)
const AdminBookingsList = () => {
    ...
    const fetchBookings = async () => {
        try {
            const res = await fetch('http://localhost:5000/api/bookings-list');
            if(res.ok) {
            ...
            console.error("Lỗi tải lịch đặt:", e);
            setLoadingBookings(false);
        }
    };
    ...
    if(loadingBookings) return <div className="p-4 text-center">Đang tải lịch đặt...</div>;
    if(bookings.length === 0) return <div className="p-4 text-center bg-white rounded-lg">Chưa có đơn đặt nào được lưu vào hệ thống.</div>;
            ...
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {bookings.map(booking => (
                        <tr key={booking.LichDatID}>
                            <td className="px-6 py-4 font-medium">{booking.TenSan}</td>
                            <td className="px-6 py-4 text-sm">{new Date(booking.NgayDat).toLocaleDateString('vi-VN')}</td>
                            <td className="px-6 py-4 text-sm">{booking.LoaiSan}</td>
                            <td className="px-6 py-4 text-sm">{booking.KhachHangName}</td>
                            <td className="px-6 py-4 text-sm">{booking.KhachHangPhone}</td>
                            <td className="px-6 py-4 text-sm"><span className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">{booking.TinhTrang}</span></td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
const FootballBookingApp = () => {
  ...
  // Fetch Busy Slots: Add fieldType to filter
  useEffect(() => {
      if(selectedField && searchFilters.date) {
          const fetchBusy = async () => {
              try {
                  const res = await fetch(`http://localhost:5000/api/check-trung-gio?sanId=${selectedField.SanID}&ngay=${searchFilters.date}&loaiSan=${bookingInfo.fieldType}`);
                  if(res.ok) { const data = await res.json(); setBusySlots(data); }
              } catch (e) { console.error(e); }
          }
          fetchBusy();
      }
  }, [selectedField, searchFilters.date, bookingInfo.fieldType]); 

  useEffect(() => { const savedUser = localStorage.getItem('currentUser'); if (savedUser) { setCurrentUser(JSON.parse(savedUser)); setIsLoggedIn(true); if(JSON.parse(savedUser).phone === 'admin') setCurrentView('admin'); } const savedHistory = localStorage.getItem('bookingHistory'); if (savedHistory) setHistory(JSON.parse(savedHistory)); fetchFields(); }, []);

  const handleLogin = (phone) => { const role = phone === 'admin' ? 'admin' : 'user'; const user = { phone: phone || '090xxxx', name: role === 'admin' ? 'Quản Trị Viên' : 'Thành viên', role }; localStorage.setItem('currentUser', JSON.stringify(user)); setCurrentUser(user); setIsLoggedIn(true); setCurrentView(role === 'admin' ? 'admin' : 'home'); };
  const handleLogout = () => { localStorage.removeItem('currentUser'); setIsLoggedIn(false); setCurrentUser(null); setCurrentView('login'); };
  ...
  const saveBooking = async () => {
      ...
      alert('Thanh toán thành công! Sân đã được lưu.'); setCurrentView('home'); setShowQR(false); 
  };

  const clearHistory = () => { if(window.confirm("Xóa lịch sử?")) { setHistory([]); localStorage.removeItem('bookingHistory'); } };
